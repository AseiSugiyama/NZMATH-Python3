...
\quad \param{a}の２乗根の小数点切り捨てた値を返す.\\ 
%\spacing
...
\spacing
% document of basi document
\quad \param{n}の\param{k}乗根の小数点切り捨てた値を返す.\\
%\spacing
% input, output document
...
\spacing
% document of basi document
\quad Legendre 記号とJacobi記号を返す $\DS \Bigl(\frac{\param{a}}{\param{m}}\Bigr)$.\\
%\spacing
% input, output document
...
\spacing
% document of basi document
\quad \param{a}の２乗根が存在する時は\param{p}を法とする\param{a}の２乗根の値を返す。さもなければエラーを返す。\\
\spacing
% add document
%\spacing
...
\spacing
% document of basi document
\quad \param{n}の\param{m}進展開を返す。.\\ 
\spacing
% input, output document
\quad \param{n}は正の整数。 \param{m}は $2$以上。.  出力は降順の係数展開のリスト。.\\
%
\subsection{inverse -- inverse}\linkedone{arith1}{inverse}
\func{inverse}{\hiki{x}{integer}, \, \hiki{p}{integer}}{\out{integer}}\\
\spacing
% document of basi document
\quad 法\param{p}における\param{x}の逆関数を返す。.\\
\spacing
% input, output document
\quad \param{p}は素数。.\\
%
...
\spacing
% input, output document
\quad 入力\param{nlist}は２つの要素からなるリスト。
一つ目は割った余りで二つ目は割る数。どちらも整数。.
\\
%
...
\spacing
% document of basi document
\quad　\param{a}と\param{b}の算術幾何平均を返す。\\
%\spacing
% input, output document
%\quad 入力\param{a}と\param{b}は整数。\\ 
%
...
\spacing
% document of basi document
\quad \param{p}進評価と\param{n}の他の部分群を返す。\\
\spacing
% added document
\quad \negok $k$が与えられたら、評価と $\param{n}p^\param{k}$の他の部分群を返す。\\
% input, output document
...
\spacing
% document of basi document
\quad \param{n}が二乗になっていたら根をを返し、さもなくば \(0\)を返す。\\
%\spacing
% input, output document
...
\spacing
% document of basi document
\quad \param{n}の対数の整数部分を返す。\param{base}.\\
%\spacing
% input, output document
...
\quad \param{iterable}のすべての要素からなるものを返す。 \\
\spacing
% added document
