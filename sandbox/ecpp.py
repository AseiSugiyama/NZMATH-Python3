...
from sandbox.config import DATADIR, HAVE_NET, PLUGIN_MATH
from sandbox.plugins import MATHMODULE as math, CMATHMODULE as cmath, \
     FLOATTYPE as Float, COMPLEXTYPE as Complex, PRECISION_CHANGEABLE, \
     SETPRECISION as setprecision
...
def round_mp(x):
    """
    Round x to nearest integer (multiprecision).
    """
    return math.floor(x + Float(0.5))
...
def dedekind(tau, floatpre):
    ...
    Output : eta(tau)
    """
    a = 2 * math.pi / Float(24)
    b = cmath.exp(Complex(0, a))
    p = 1
    m = 0
    while m <= Float(0.999):
        n = round_mp(tau.real)
        ...
        if m <= Float(0.999):
            ro = cmath.sqrt(pow(tau, -1)*Complex(0, 1))
            if ro.real < 0:
                ro = -ro
            p = p*ro
            tau = Complex(-p.real, p.imag) / m
    q1 = cmath.exp(a*tau*Complex(0, 1))
    q = q1**24
    s = 1
    qs = Complex(1, 0)
    qn = 1
    des = Float(10)**(-floatpre)
    while abs(qs) > des:
        t = -q*qn*qn*qs
        qn = qn*q
        qs = qn*t
...
def hilbert_multiprecision(D):
    """
    Algorithm 23&24 (Floating-point precision & Hilbert class polynomial)
    Input : a negative fundamental discriminant D
    Output : the Hilbert class polynomial T in Z[X], the class number
             h(D) and the set of reduced forms
    """
    if not PRECISION_CHANGEABLE:
        raise NotImplementedError("doesn't work without multiprecision")
    if D >= 0:
        raise ValueError("Input number must be a negative integer.")
    T = [Complex(1), Complex(0)]
    setprecision(53) # default precision
    h, reduced_forms = quad.class_group(D)
    harmonic_sum = sum(1/Float(form[0]) for form in reduced_forms)
    floatpre = round_mp(math.pi*math.sqrt(-D)*harmonic_sum / math.log(10)) + 10
    setprecision(math.ceil(floatpre*math.log(10, 2)))
    sqrtD = cmath.sqrt(D)
    for a, b, c in reduced_forms:
        if b < 0:
            continue
        tau = (-b + sqrtD)/(2*a)
        f = cmath.pow(dedekind(2*tau, floatpre) / dedekind(tau, floatpre), 24)
        j = ((256*f + 1)**3) / f
        TC = tuple(T)
    ...
    T = [(int(round_mp(c.real)) if hasattr(c, 'real') else c) for c in T]
    return h, T

def hilbert(D):
    """
    wrapper to split multiprecision part and others.
    """
    if HAVE_NET:
        import urllib2
    ...
    if PLUGIN_MATH is not None:
        return hilbert_multiprecision(D)
