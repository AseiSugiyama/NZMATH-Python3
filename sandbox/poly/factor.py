...
import nzmath.sandbox.poly.hensel as hensel
...
def zassenhaus(f):
    ...
    #while q < 2*blm:
    #    lf_inv_modq = intresidue.IntegerResidueClass(lf,q*p).inverse()
    #    fq = f.coefficients_map(lambda c: (lf_inv_modq*c).minimumAbsolute())
    #    print fq, q*p
    #    fp_factors = padic_lift_list(fq, fp_factors, p, q)
    #    print fp_factors
    #    q *= p
